...
import MyCheckbox from './component/Checkbox/MyCheckbox'
  ...
  return (
    <AppStyle>
      ...
      <div>
        <MyInput
          value={taskName}
          onChange={(e: React.ChangeEvent<HTMLInputElement>): void => setTaskName(e.target.value)}
      ...
      <div>
        {filteredTasks.map((item) =>
          <div key={item.id} className="item-task">
            <MyCheckbox completed={item.completed} onToggle={(): void => handleToggleTask(item.id)}/>
            <span>{item.name}</span>
          </div>
        )}
      </div>
